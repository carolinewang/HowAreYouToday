...
public class Crying extends Activity {
	...
	private String[] tags;
	private int moodCategory = 6;
	private int ifWantComfort = 0;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_crying);
...
//				}else Toast.makeText(Sad.this, "null", Toast.LENGTH_SHORT).show();
	...
	protected void launchTagEditorActivity() {
		Intent intent = new Intent(Crying.this, TagEditorActivity.class);
...
//		mood.saveEventually();
		mood.saveInBackground(new SaveCallback() {
			...
			public void done(ParseException e) {
				if (e == null) {
					Toast.makeText(Crying.this, R.string.toast_record_mood, Toast.LENGTH_SHORT).show();
					Intent intent = new Intent(Crying.this, MainActivity.class);
					intent.putExtra(Application.MOOD_CATEGORY, moodCategory);
					intent.putExtra(Application.MOOD_DESCRIPTION, moodDescription.getText().toString());
					intent.putExtra(Application.TAGCONTENTS, tags);
					intent.putExtra(Application.DATE, date.getTime());
					...
					Toast.makeText(Crying.this, e.toString(), Toast.LENGTH_SHORT).show();
				}
			}
		});
